...
const Navbar: FC<NavbarProps> = ({ updateMoviesList }) => {
  ...
  return (
    <div className="movie_navbar">
      <Link className="home_nav_link" to="/">
        Movie Hub
        <img 
          className="home_page_img" 
          src="https://cdn-icons-png.flaticon.com/128/3172/3172625.png" 
          alt="Movie Hub Logo"
        />
      </Link>
      <Link className="movie_nav_link" to="/saved">Saved</Link>
      <Link className="movie_nav_link" to="/history">History</Link>
      {location.pathname === '/' && <MovieForm key="movie-form" updateMoviesList={updateMoviesList} />}
